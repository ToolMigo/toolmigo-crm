...
def build_quote_pdf(quote):
    ...
    org=quote.organization; accent=colors.HexColor(org.primary_color)
    ...
    story += [Paragraph(f'<font color="{escape(org.primary_color)}" size="8"><b>OFFERTE</b></font>',body),Paragraph(escape(quote.title),ParagraphStyle('title',parent=styles['Title'],fontName='Helvetica-Bold',fontSize=25,leading=29,textColor=INK,spaceAfter=8*mm))]
...
def build_invoice_pdf(invoice):
    ...
    org=invoice.organization; accent=colors.HexColor(org.primary_color); fallback=Path(settings.BASE_DIR)/'static'/'img'/'toolmigo-logo.jpeg'; logo_path=Path(org.logo.path) if org.logo and Path(org.logo.path).is_file() else fallback
