...
def build_quote_pdf(quote):
    ...
    org=quote.organization; template=org.document_templates.filter(kind='quote',is_default=True).first(); accent_hex=template.primary_color if template else org.primary_color; accent=colors.HexColor(accent_hex)
    fallback=Path(settings.BASE_DIR)/'static'/'img'/'toolmigo-logo.jpeg'; logo_path=Path(org.logo.path) if org.logo and Path(org.logo.path).is_file() else fallback; logo=Image(str(logo_path),17*mm,17*mm)
    brand=Paragraph(f'<font size="19" color="{escape(org.primary_color)}"><b>{escape(org.name)}</b></font><br/><font size="7" color="#667085">CRM</font>',body)
    org_text='<b>%s</b><br/>%s<br/>%s %s<br/>KvK %s · BTW %s' % tuple(escape(str(x or '—')) for x in (org.name,org.address,org.postal_code,org.city,org.kvk_number,org.vat_id))
    ...
    if template and template.header: story += [Paragraph(escape(template.header).replace('\n','<br/>'),small),Spacer(1,4*mm)]
    story += [Paragraph(f'<font color="{escape(accent_hex)}" size="8"><b>OFFERTE</b></font>',body),Paragraph(escape(quote.title),ParagraphStyle('title',parent=styles['Title'],fontName='Helvetica-Bold',fontSize=25,leading=29,textColor=INK,spaceAfter=8*mm))]
    ...
    table=Table(rows,colWidths=[76*mm,25*mm,27*mm,17*mm,27*mm],repeatRows=1); table.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),accent),('TEXTCOLOR',(0,0),(-1,0),colors.white),('VALIGN',(0,0),(-1,-1),'TOP'),('GRID',(0,1),(-1,-1),.25,colors.HexColor('#E7ECEA')),('TOPPADDING',(0,0),(-1,-1),7),('BOTTOMPADDING',(0,0),(-1,-1),7),('ALIGN',(1,1),(-1,-1),'RIGHT')]))
    ...
    if template and template.footer: story += [Spacer(1,6*mm),Paragraph(escape(template.footer).replace('\n','<br/>'),small)]
    def page(canvas,doc):
        canvas.saveState(); canvas.setStrokeColor(LIGHT); canvas.line(19*mm,13*mm,191*mm,13*mm); canvas.setFillColor(MUTED); canvas.setFont('Helvetica',7); canvas.drawString(19*mm,8*mm,org.name); canvas.drawRightString(191*mm,8*mm,f'Pagina {doc.page}'); canvas.restoreState()
...
def build_invoice_pdf(invoice):
    ...
    org=invoice.organization; template=org.document_templates.filter(kind='invoice',is_default=True).first(); accent_hex=template.primary_color if template else org.primary_color; accent=colors.HexColor(accent_hex); fallback=Path(settings.BASE_DIR)/'static'/'img'/'toolmigo-logo.jpeg'; logo_path=Path(org.logo.path) if org.logo and Path(org.logo.path).is_file() else fallback
    logo=Image(str(logo_path),17*mm,17*mm); brand=Paragraph(f'<font size="19" color="{escape(org.primary_color)}"><b>{escape(org.name)}</b></font><br/><font size="7">CRM</font>',body)
    ...
    story=[Table([[logo,brand,Paragraph(org_text,right)]],colWidths=[20*mm,70*mm,82*mm],style=[('VALIGN',(0,0),(-1,-1),'TOP'),('ALIGN',(-1,0),(-1,-1),'RIGHT')]),Spacer(1,12*mm)]
    if template and template.header: story += [Paragraph(escape(template.header).replace('\n','<br/>'),small),Spacer(1,4*mm)]
    story += [Paragraph(f'<font color="{escape(accent_hex)}" size="8"><b>{label}</b></font>',body),Paragraph(escape(invoice.title),ParagraphStyle('ititle',parent=styles['Title'],fontName='Helvetica-Bold',fontSize=25,textColor=INK,spaceAfter=7*mm))]
    ...
    def page(canvas,doc): canvas.saveState(); canvas.setStrokeColor(LIGHT); canvas.line(19*mm,13*mm,191*mm,13*mm); canvas.setFillColor(MUTED); canvas.setFont('Helvetica',7); canvas.drawString(19*mm,8*mm,org.name); canvas.drawRightString(191*mm,8*mm,f'Pagina {doc.page}'); canvas.restoreState()
    doc.build(story,onFirstPage=page,onLaterPages=page); return out.getvalue()
